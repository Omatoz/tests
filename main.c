...
void afficherPlateau(char plateau[SIZE][SIZE][4], int barrierX, int barrierY, int modePlacement1, int modePlacement2, char orientation) {
#ifdef _WIN32
    ...
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            ...
            if ((modePlacement1 || modePlacement2)) {
                if (orientation == 'V') {
...
int main() {
    ...
    int x3 = 9, y3 = 1; // Position de départ du joueur 3 (diagonale)
    int x4 = 9, y4 = SIZE - 2; // Position de départ du joueur 4 (diagonale)
    int tour = 1;
